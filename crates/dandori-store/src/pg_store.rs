use chrono::Utc;
use dandori_domain::{
    AuthContext, CreateIssueCommandV1, DomainError, Issue, IssueCreatedEventV1, IssuePriority,
    IssueStateCategory,
};
...
impl PgStore {
    ...
    pub async fn create_issue_transactional(
        ...
        set_workspace_context(&mut tx, auth.workspace_id.0).await?;
        ...
        sqlx::query(
            "INSERT INTO issue (
                ...
                created_at,
                updated_at
            ) VALUES ($1, $2, $3, $4, $5, $6, 'open', $7, NULL, 0, $8, $8)",
        )
        ...
        sqlx::query(
            "INSERT INTO outbox (
                ...
                status,
                created_at
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 0, $10, 'pending', $10)",
        )
        .bind(Uuid::now_v7())
        .bind(command.workspace_id.0)
        .bind(event.event_id)
...
async fn set_workspace_context(
